...
Execute pre-push hooks as defined in githook-config.json using make commands.

## Instructions

Execute the following steps to run pre-push checks:

### 1. Read Git Hook Configuration
- Read the `githook-config.json` file from the project root
- Parse the JSON configuration to extract the "pre-push" array
- If the file doesn't exist or "pre-push" key is missing, inform the user and exit

### 2. Execute Pre-Push Commands
- For each command in the "pre-push" array, execute it as a make command
...
- Provide clear feedback on the overall pre-push status
...
- `/pre-push` - Runs all pre-push hooks defined in githook-config.json
